...
fn expand_posix_keystring(args: Vec<std::ffi::OsString>) -> Vec<std::ffi::OsString> {
    ...
    for c in key.chars() {
        match c {
            ...
            'b' => {
                // The 'b' modifier is not yet implemented; consume its file
                // operand per the POSIX spec so that subsequent operands
                // (e.g. the archive name for 'f') are still positioned correctly.
                if file_idx < file_operands.len() {
                    file_idx += 1;
                }
            }
