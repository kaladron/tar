...
pub fn extract_archive(archive_path: &Path, verbose: bool) -> UResult<()> {
    ...
    for entry_result in archive.entries().map_err(|e| {
        TarError::InvalidArchive(format!("Failed to read archive entries: {}", e))
    })? {
        let mut entry = entry_result.map_err(|e| {
            TarError::InvalidArchive(format!("Failed to read entry: {}", e))
        })?;
        ...
        let path = entry
            .path()
            .map_err(|e| {
                TarError::InvalidArchive(format!("Failed to read entry path: {}", e))
            })?
            .to_path_buf();
