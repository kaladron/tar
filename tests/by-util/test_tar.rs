...
fn test_verbose() {
    ...
    let dir1_path = PathBuf::from("dir1");
    let dir2_path = dir1_path.join("dir2");
    let file1_path = dir1_path.join("file1.txt");
    let file2_path = dir2_path.join("file2.txt");

    let dir1_str = dir1_path.to_string_lossy().into_owned();
    let dir2_str = dir2_path.to_string_lossy().into_owned();
    let file1_str = file1_path.to_string_lossy().into_owned();
    let file2_str = file2_path.to_string_lossy().into_owned();

    at.mkdir(&dir1_path);
    at.mkdir(&dir2_path);
    at.write(&file1_str, "test content 1");
    at.write(&file2_str, "test content 2");

    ucmd.args(&["-cvf", "archive.tar", &dir1_str])
        .succeeds()
        .stdout_contains(&dir1_str)
        .stdout_contains(&dir2_str)
        .stdout_contains(&file1_str)
        .stdout_contains(&file2_str);
}
